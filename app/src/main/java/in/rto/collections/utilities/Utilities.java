...
//            return !ipAddr.equals("");
    ...
    public static boolean isVehicleNo(EditText edt) {
        edt.setError(null);
        if ((edt.getText().toString().trim().length() <= 10) && (isValidVehicleno(edt.getText().toString().trim())))
            return true;
        else {
            return false;
        }
    ...
    private static boolean isValidVehicleno(String mobileno) {
        String Mobile_PATTERN = "^[A-Z]{2}[0-9]{2}[A-Z0-9]*$";                                               //^[+]?[0-9]{10,13}$
        Pattern pattern = Pattern.compile(Mobile_PATTERN);
        Matcher matcher = pattern.matcher(mobileno);
        return matcher.matches();
    }
