...
public class Fragment_Other_Vehicle_Details extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment__other__vehicle__details, container, false);
        context = getActivity();
        init(rootView);
        getSessionData();
        setDefault();
...
//        shimmer_view_container = rootView.findViewById(R.id.shimmer_view_container);
    ...
    private void setDefault() {
        if (Utilities.isNetworkAvailable(getActivity())) {
            new GetOtherVehicleDealerList().execute(user_id);
        } else {
    ...
    private void setEventHandlers() {
        fab_add_dealer.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(context, AddOtherVehicleDetails_Activity.class));
            }
        });
...
//            shimmer_view_container.startShimmer();
        ...
        protected String doInBackground(String... params) {
            ...
            res = WebServiceCalls.FORMDATAAPICall(ApplicationConstants.OTHERVEHICLEDEALER, param);
            return res.trim();
        }
                    ...
                    if (type.equalsIgnoreCase("success")) {
                        ...
                        if (jsonarr.length() > 0) {
                            for (int i = 0; i < jsonarr.length(); i++) {
                                ...
                                dealerMainObj.setId(jsonObj.getString("vehicle_details_id"));
                                ...
                                dealerMainObj.setImportR("Import");
                                ...
                                ArrayList<VehicleDealerListPojo.WheelDatesListPojo> wheelDatesListPojos = new ArrayList<>();

                                for (int j = 0; j < jsonObj.getJSONArray("wheel_alignment_dates").length(); j++) {
                                    VehicleDealerListPojo.WheelDatesListPojo otherdateobj = new VehicleDealerListPojo.WheelDatesListPojo();
                                    otherdateobj.setWheel_alignment_date(jsonObj.getJSONArray("wheel_alignment_dates").getJSONObject(j).getString("alignment_date"));
                                    otherdateobj.setText(jsonObj.getJSONArray("wheel_alignment_dates").getJSONObject(j).getString("text"));
                                    otherdateobj.setWheel_date_id(jsonObj.getJSONArray("wheel_alignment_dates").getJSONObject(j).getString("wheel_id"));
                                    wheelDatesListPojos.add(otherdateobj);
                                }
